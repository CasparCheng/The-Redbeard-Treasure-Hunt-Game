...
class PriorityQueue(Container):
    ...
    (FIFO) order, meaning the item
    which was inserted *earlier* is the first one
    to be removed.
    ...
    def add(self, item):
        ...
        # loop the element in the queue to get the element with index
        for i, ele in enumerate(self._queue):
            # compare the priority of the inputed item and element
            if not self._less_than(item, ele):
                # if the item has higher priority than the element
                # then insert the item in that index
                self._queue.insert(i, item)
                break
        # otherwise
        else:
            # add the item to the common order
